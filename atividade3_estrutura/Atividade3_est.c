...
void inserir(Fila *f, Livro l) {
  if (f->nItens < f->capacidade) {
      ...
      printf("Fila lotada. Capacidade maxima e 50 livros.\n");
  }
}
...
void empilhar(struct Pilha *p, Livro l, const char* arquivo) {
    if (p->topo < p->capacidade - 1) {
        if (strlen(l.autor) > 0 && strlen(l.titulo) > 0) {
            ...
            printf("Livro vazio. Não foi possivel empilhar.\n");
        }
    } else {
        printf("Pilha lotada. Capacidade maxima e de 50 livros.\n");
    }
}
...
void removerLivroPorTituloDaPilha(Pilha *p, const char* arquivo) {
    ...
    char titulo[100];
    printf("Digite o título do livro que sera removido: ");
    getchar();
    ...
    switch(op){
      case 1:
        ...
        if(estaVazia(&f)){
          ...
          printf("Fila lotada. Capacidade maxima e de 50 livros.\n");
        }else{
      ...
      case 3:
        ...
        if (strlen(autor) > 0 && strlen(titulo) > 0) {
            ...
            if (!pilhaCheia(&p)) {
                ...
                printf("Pilha cheia. Capacidade maxima e de 50 livros.\n");
            }
        } else {
